# -*- coding: utf-8 -*-
"""
第二步：将解析好的 JSON 数据渲染成互动教学型单文件 HTML。
"""
import json
import os
import subprocess
...
# 课程输出到 project/ 下，与其它课程统一；这样 build-gallery.sh 扫描时会自动收录。
OUT = "/Users/zhangxiang/github/codebase-to-course/project/pojv-course/index.html"

with open("/tmp/pojv_data.json", "r", encoding="utf-8") as f:
    DATA = json.load(f)
...
ELEM_KEY_MAP = {m[0]: (m[1], m[2], m[3]) for m in ELEMENT_META}
...
# ---------- 渲染知识点 ----------
def render_point(pt, idx):
    aid = slug(pt["id"], pt["title"])
    # 元素排序按固定顺序
    ...
    # 习题
    ex_html = render_exercises(pt)
    ex_count = len(pt.get("exercises", []))

    return f"""
  <section class="kp" id="{aid}" data-module="{pt['module_num']}" data-title="{esc(pt['title'])}" data-diff="{esc(ex_count)}">
    <div class="kp-anchor"><a href="#{aid}" class="anchor-link">#</a></div>
    <div class="kp-tag">知识点 {esc(pt['id'])} · 模块{pt['module_num']}</div>
...
# ---------- 渲染习题 ----------
def render_exercises(pt):
    exs = pt.get("exercises", [])
    if not exs:
        return ""
    pid = pt["id"]
    blocks = []
    for ex in exs:
        ...
        ans = pt.get("answers", {}).get(str(ex["num"]), "")
        ans_html = ""
        if ans:
            ans_html = f"""
        <button class="ex-toggle" data-target="{qid}-ans" data-opened="false">查看解析 →</button>
    ...
    return f"""
    <div class="exercises">
      <div class="exercises-head">📝 本节配套习题（{len(exs)} 题）</div>
      {''.join(blocks)}
    </div>"""

# ---------- 渲染章节 ----------
def render_chapter(ch):
    pts = "".join(render_point(pt, i) for i, pt in enumerate(ch["points"]))
    return f"""
...
def render_module(mod):
  ...
  <div class="module" id="mod-{mod['num']}">
    <header class="module-header">
      <div class="module-badge">模块 {mod['num']}</div>
...
def render_sidebar():
    ...
    for mod in DATA["modules"]:
        ...
        for ch in mod["chapters"]:
            sub = "".join(
                f'<li><a href="#{slug(pt["id"], pt["title"])}" class="nav-point">{esc(pt["id"])} {esc(pt["title"])}</a></li>'
                for pt in ch["points"]
            )
            children.append(f"""
          <li>
...
:root{{
  ...
  --primary:#4f46e5; --primary-2:#6366f1; --primary-soft:#eef0ff;
  --accent:#ec4899;
  --shadow:0 1px 3px rgba(20,25,50,.06),0 1px 2px rgba(20,25,50,.04);
  --shadow-lg:0 10px 40px -10px rgba(20,25,50,.15);
  --radius:14px;
  --c-def:#4f46e5; --c-why:#0891b2; --c-how:#7c3aed; --c-feynman:#d97706;
  --c-scene:#059669; --c-pitfall:#dc2626;
  --c-def-bg:#eef0ff; --c-why-bg:#ecfeff; --c-how-bg:#f5f3ff;
...
[data-theme="dark"]{{
  ...
  --primary:#818cf8; --primary-2:#a5b0fc; --primary-soft:#262a3d;
  --shadow:0 1px 3px rgba(0,0,0,.3);
  --shadow-lg:0 10px 40px -10px rgba(0,0,0,.5);
  --c-def-bg:#1e1e3a; --c-why-bg:#0c2a33; --c-how-bg:#221638;
...
code{{background:var(--surface-2);padding:2px 6px;border-radius:4px;font-size:.9em;font-family:"SF Mono",Consolas,monospace}}
...
.topbar-logo .logo-mark{{width:34px;height:34px;border-radius:9px;background:linear-gradient(135deg,var(--primary),var(--accent));display:flex;align-items:center;justify-content:center;color:#fff;font-size:18px;font-weight:800}}
...
.nav-module.open .nav-chapters{{max-height:2000px}}
.nav-chapter{{
...
.elem-def{{background:var(--c-def-bg);border-color:rgba(79,70,229,.25)}}.elem-def .elem-label{{color:var(--c-def)}}
...
.ex-ans p{{margin-bottom:8px}}.ex-ans p:last-child{{margin-bottom:0}}
...
/* 搜索遮罩 */
.search-empty{{padding:60px 20px;text-align:center;color:var(--text-3)}}
...
<div class="layout">
  ...
  <main class="main">
    <section class="hero">
      <span class="hero-eyebrow">企业内生性增长 · 方法论</span>
      <h1>{esc(DATA['title'])}</h1>
      <p class="hero-sub">{esc(DATA['subtitle'])} · 系统化掌握破局的底层逻辑与 22 个落地工具</p>
      <div class="hero-stats">
        <div class="hero-stat"><div class="num">{stats['modules']}</div><div class="lbl">核心模块</div></div>
        ...
        <div class="hero-stat"><div class="num">2</div><div class="lbl">思维导图</div></div>
      </div>
    </section>
...
(function(){{
  ...
  const saved=localStorage.getItem('pojv-theme');
  if(saved){{body.setAttribute('data-theme',saved);themeBtn.textContent=saved==='dark'?'☀️':'🌙';}}
  themeBtn.addEventListener('click',()=>{{
    const cur=body.getAttribute('data-theme')==='dark'?'light':'dark';
    body.setAttribute('data-theme',cur);localStorage.setItem('pojv-theme',cur);
  ...
  const navModules=document.querySelectorAll('.nav-module-link');
  ...
  document.querySelectorAll('.ex-toggle').forEach(btn=>{{
    btn.addEventListener('click',function(){{
      ...
      this.textContent=opened?'查看解析 →':'收起解析 ↑';
    }});
  }});
  ...
  // mermaid
  if(window.mermaid){{
    mermaid.initialize({{startOnLoad:true,theme:body.getAttribute('data-theme')==='dark'?'dark':'default',securityLevel:'loose'}});
  }}
}})();
</script>
</body>
...
# 自动刷新看板：与 ecc-course/build.sh 结尾调用 build-gallery.sh 的模式一致，
# 让 project/index.html 立即收录新课程，无需手动再跑一次。
